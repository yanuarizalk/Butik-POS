<?php
    ...
    $title = "Penambahan / Pengurangan Inventory";
...
<div class="page">
	...
	<div class="page-40">
		<table class="commonTable">
			<tr>
				<td style="width: 25%;">Aktifitas *</td>
				<td style="width: 75%;">
					<select name="act" style="width: 50%; max-width: 170px;" required>
			...
			<tr>
				<td>Toko *</td>
				<td>
					<select name="id_toko" style="width: 50%; max-width: 170px;" required>
...
$(document).ready(function() {
	...
	$('#editor').on('submit', function(ev) {
		...
		if (!$('#preProduk').has('#qty').length) {
			let act = $('select[name="act"]').val() == 'IN' ? 'Ditambahkan' : 'Dikurangkan';
			showAlert('', 'Mohon inputkan Produk yang ingin ' + act);
			return;
		}
		$.ajax({
			'url': '<?php echo BASE_URL; ?>api.php?data=product_inventory&act=add',
			'method': 'POST',
			'data': submit,
			'success': function(data) {
				if (data.status == 'success') {
					showAlert('', 'Inventory berhasil diupdate');
